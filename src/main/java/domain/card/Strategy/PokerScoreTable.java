...
public class PokerScoreTable implements ScoreTable{

    private final static Map<Signature, Integer> POKER_SCORING = Map.ofEntries(
            Map.entry(Signature.ACE,1),
            ...
            Map.entry(Signature.JACK,11),
            Map.entry(Signature.QUEEN,12),
            Map.entry(Signature.KING,13)
    );

    @Override
    public int getScore(Signature signature) {
        return POKER_SCORING.get(signature);
    }
}
